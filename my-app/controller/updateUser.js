...
async function createUser(req, res, next) {
    ...
    if(password && password.length) {
        dataToUpdate.password = CryptoJS.SHA256(config.encryptionKey+Buffer.from(password, 'base64').toString('ascii')).toString();
        dataToUpdate.account_pass = encrypt(Buffer.from(password, 'base64').toString('ascii'));
    }
    ...
    let verifyExitingUser = await authenticationModel
    ...
    if (verifyExitingUser && verifyExitingUser.length > 0) {
        ...
        Object.keys(account_details).forEach(key => {
            if(dataToUpdate.account_details[key]) {
                dataToUpdate.account_details[key] = dataToUpdate.account_details[key];
            } else {
